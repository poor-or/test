...
const points = [];
function getPoint() {
    const x = Math.ceil(Math.random() * canvas.width), // 粒子的x坐标
        y = Math.ceil(Math.random() * canvas.height), // 粒子的y坐标
        r = (Math.random() * 5).toFixed(4), // 粒子的半径
        speed = (Math.random() * 3).toFixed(4), //运动速率;
        directionAngle = Math.floor(Math.random()*360),//运动方向;
        rateX = speed*Math.cos(directionAngle),// 粒子在x方向运动的速率
        rateY = speed*Math.sin(directionAngle); // 粒子在y方向运动的速率
    return { x, y, r,speed,directionAngle, rateX, rateY };
}
for (let i = 0; i < 100; i++) {
