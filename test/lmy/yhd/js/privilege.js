...
var userInfo1 = document.getElementById("user-name");
var userName = sessionStorage.getItem("userName");
var rights=document.getElementById("user-rights").children;
if (userName) {
    // 查用户等级
    function getLv() {
        var xhr = new XMLHttpRequest();
                ...
                for(var i=0;i<data.length;i++){
                    if(data[i].MemberName==userName){
                        ...
                        for(var j=0;j<lvList[userLv-1];j++){
                            rights[j].children[0].children[0].style.display="block";
                            rights[j].children[0].children[1].classList.add("green");
                            rights[j].children[1].children[0].style.color="#bf9e72";
                        }
                    }
                }
            }
    ...
    userInfo1.children[0].innerHTML = userName;
} else {
    userInfo[0].parentElement.previousElementSibling.style.display = "block";
    userInfo[0].parentElement.style.display = "none";
    userInfo1.innerHTML="赶紧来登录吧~";
    userInfo1.previousElementSibling.children[0].src="./img/imgs/user_default.jpg";
}
function exitUser() {
    userInfo[0].parentElement.style.display = "none";
    userInfo[0].parentElement.previousElementSibling.style.display = "block";
